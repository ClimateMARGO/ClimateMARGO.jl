...
function plot_emissions(m::ClimateModel)
    ...
    fill_between(t(m), effective_emissions(m), effective_emissions(m, M=true), facecolor="C0", alpha=0.3, label=L"$rq(1-M)$")
    fill_between(t(m), effective_emissions(m, M=true), effective_emissions(m, M=true, R=true), facecolor="C1", alpha=0.3, label=L"$rq(1-M) - rq_{0}R$")
    ...
    plot(t(m), effective_emissions(m, M=true, R=true), "k-", lw=2.25)
...
function plot_concentrations(m::ClimateModel)
    ...
    fill_between(t(m), c(m), c(m, M=true), facecolor="C0", alpha=0.3, label=L"$c_{M}$")
    fill_between(t(m), c(m, M=true), c(m, M=true, R=true), facecolor="C1", alpha=0.3, label=L"$c_{M,R}$")
    plot(t(m), c(m), "-", color="grey", lw=2.25, label=L"$c$ (no-policy baseline)")
    plot(t(m), c(m, M=true), "k-", lw=1, alpha=0.4)
    plot(t(m), c(m, M=true, R=true), "k-", lw=2.25)
...
function plot_forcing(m::ClimateModel)
    ...
    fill_between(t(m), F(m), F(m, M=true), facecolor="C0", alpha=0.3, label=L"$F_{M}$")
    fill_between(t(m), F(m, M=true), F(m, M=true, R=true), facecolor="C1", alpha=0.3, label=L"$F_{M,R}$")
    fill_between(t(m), F(m, M=true, R=true), F(m, M=true, R=true, G=true), facecolor="C3", alpha=0.3, label=L"$F_{M,R,G}$")
    ...
    plot(t(m), F(m, M=true, R=true, G=true), "k-", lw=2.25)
    ylims = [0., maximum(c(m))*1.05]
...
function plot_temperatures(m::ClimateModel)
    ...
    fill_between(t(m), T(m), T(m, M=true), facecolor="C0", alpha=0.3, label=L"$T_{M}$")
    fill_between(t(m), T(m, M=true), T(m, M=true, R=true), facecolor="C1", alpha=0.3, label=L"$T_{M,R}$")
    fill_between(t(m), T(m, M=true, R=true), T(m, M=true, R=true, G=true), facecolor="C3", alpha=0.3, label=L"$T_{M,R,G}$")
    fill_between(t(m), T(m, M=true, R=true, G=true), T(m, M=true, R=true, G=true, A=true), facecolor="C2", alpha=0.3, label=L"$T_{M,R,G,A}$")
    ...
    plot(t(m), T(m, M=true, R=true, G=true, A=true), "k-", lw=2.25)
...
function plot_benefits(m::ClimateModel; discounting=true)
    ...
    plot(t(m)[domain_idx], benefit(m, discounting=discounting, M=true, R=true, G=true, A=true)[domain_idx], color="C1", lw=2, label="benefits (of avoided damages)")
    plot(t(m)[domain_idx], cost(m, discounting=discounting, M=true, R=true, G=true, A=true)[domain_idx], color="C3", lw=2, label="costs (of climate controls)")
...
function plot_damages(m::ClimateModel; discounting=true, percent_GWP=false)
    ...
    plot(t(m)[domain_idx], (damage(m, discounting=discounting) ./ Enorm)[domain_idx], color="gray", lw=2, label="uncontrolled damages")
    plot(t(m)[domain_idx], ((damages .+ costs)./ Enorm)[domain_idx], color="k", lw=2, label="net costs (controlled damages + controls)")
    plot(t(m)[domain_idx], (damages ./ Enorm)[domain_idx], color="C1", lw=2, label="controlled damages")
    plot(t(m)[domain_idx], (costs ./ Enorm)[domain_idx], color="C3", lw=2, label="cost of controls")
